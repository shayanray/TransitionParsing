...
class ParsingSystem:
    ...
    def apply(self, c, t):
        ...
        topWordFromStk = c.getStack(0)
        secondTopWordFromStk = c.getStack(1)

        lbl =  t[2:-1]  # take index from '(' and upto but not including ')'

        # left shift
        if t[0] == 'L':
            c.addArc(topWordFromStk, secondTopWordFromStk, lbl)
            c.removeSecondTopStack() # remove the word to which the arc points to

        # right shift
        elif t[0] == 'R':
            c.addArc(secondTopWordFromStk, topWordFromStk, lbl)
            c.removeTopStack() # remove the word to which the arc points to
