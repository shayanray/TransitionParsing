...
class DependencyParserModel(object):
    ...
    def build_graph(self, graph, embedding_array, Config):
        ...
        with graph.as_default():
            ...
            weights_input_3 = tf.Variable(tf.random_normal([Config.hidden_size,Config.hidden_size ],stddev=stddev_calc))
            ...
            # output of 1st layer is input to next layer and so on
            pred_y1 = self.forward_pass(flattend_trn_embdngs, weights_input_1, biases_input_1, weights_input_2)
            pred_y2 = self.forward_pass(pred_y1, weights_input_2, biases_input_2, weights_input_3)
            self.prediction = self.forward_pass(pred_y2, weights_input_3, biases_input_3, weights_output)
            ...
            l2_loss = tf.nn.l2_loss(weights_input_1) + tf.nn.l2_loss(biases_input_1) + tf.nn.l2_loss(weights_input_2) + tf.nn.l2_loss(biases_input_2) + tf.nn.l2_loss(weights_input_3) + tf.nn.l2_loss(biases_input_3) + tf.nn.l2_loss(flattend_trn_embdngs) + tf.nn.l2_loss(weights_output) 
            ...
            # output of 1st layer is input to next layer and so on
            pred_y1_test = self.forward_pass(test_embed, weights_input_1, biases_input_1, weights_input_2)
            pred_y2_test = self.forward_pass(pred_y1_test, weights_input_2, biases_input_2, weights_input_3)
            self.test_pred = self.forward_pass(pred_y2_test, weights_input_3, biases_input_3, weights_output)
